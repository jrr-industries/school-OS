...
export function SchoolsModule() {
  ...
  const columns = useMemo<Column<School>[]>(
    () => [
      ...
      },
      {
        ...
        render: (s) => <span className="text-sm font-medium text-foreground">{s.students.toLocaleString()}</span>,
      },
      {
        key: 'teachers',
        header: 'Teachers',
        ...
        render: (s) => <span className="text-sm text-foreground">{s.teachers}</span>,
      },
      {
        key: 'status',
        header: 'Status',
      ...
      },
      {
        ...
        render: (s) => (
          <Badge
            variant={s.subscription === 'premium' ? 'default' : s.subscription === 'standard' ? 'info' : 'secondary'}
            className="h-6 px-2 text-[10px]"
          >
            {s.subscription}
          </Badge>
      ...
      },
      {
        ...
        sortable: true,
        className: 'text-right',
        render: (s) => <span className="text-sm font-medium text-foreground">₹{s.revenue.toLocaleString()}</span>,
      },
      {
        key: 'actions',
        header: '',
  ...
  return (
    <PageContainer>
      <PageHeader title="Schools" description="Manage all registered schools in the system">
