...
export function AttendanceModule() {
  ...
  const [classFilter, setClassFilter] = useState((savedState.classFilter as string) || 'all');

  const handleSearchChange = useCallback(
    (value: string) => {
      setSearch(value);
      setModuleState(MODULE_PATH, { search: value, classFilter });
    },
    [classFilter, setModuleState],
  );

  const handleClassFilterChange = useCallback(
    (value: string) => {
      setClassFilter(value);
      setModuleState(MODULE_PATH, { search, classFilter: value });
    },
    [search, setModuleState],
  );
  ...
  const columns = useMemo<Column<AttendanceRecord>[]>(
    () => [
      ...
      },
      {
        key: 'markedBy',
        header: 'Marked By',
        render: (r) => <span className="text-sm text-muted-foreground">{r.markedBy}</span>,
        hideOnMobile: true,
      },
    ],
    [],
