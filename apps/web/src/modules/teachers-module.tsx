...
export function TeachersModule() {
  ...
  const columns = useMemo<Column<Teacher>[]>(
    () => [
      ...
      },
      {
        ...
        header: 'Department',
        sortable: true,
        render: (t) => <span className="text-sm text-foreground">{t.department}</span>,
      },
      {
        key: 'subjects',
        header: 'Subjects',
      ...
      },
      {
        ...
        sortable: true,
        render: (t) => <span className="text-sm text-foreground">{t.qualification}</span>,
        hideOnMobile: true,
      },
      {
        key: 'experience',
      ...
      },
      {
        ...
        render: (t) => <span className="text-sm text-foreground">{t.students}</span>,
      },
      {
        key: 'status',
        header: 'Status',
        ...
        render: (t) => (
          <Badge
            variant={t.status === 'active' ? 'success' : t.status === 'on-leave' ? 'warning' : 'secondary'}
  ...
  return (
    <PageContainer>
      <PageHeader title="Teachers" description="Manage teacher profiles and assignments">
