...
export default function PendingVerificationPage() {
  ...
  const handleApprove = async (school: SchoolRecord) => {
  ...
  const handleReject = async (school: SchoolRecord) => {
    setProcessingId(school.id);
    try {
      const res = await fetch(`/api/admin/schools/${school.id}`, {
        method: 'PATCH',
  ...
  const renderEmpty = () => (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h3 className="mb-1 text-lg font-semibold">No pending verifications</h3>
      <p className="text-sm text-muted-foreground">
        {search ? 'No schools match your search' : 'All schools have been verified'}
      </p>
    </div>
  );
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Review and approve schools waiting for verification
            </p>
          </div>
        ...
        <CardContent>
          ...
          {loading ? renderSkeleton() : error ? renderError() : schools.length === 0 ? renderEmpty() : (
            <>
                ...
                <table className="w-full caption-bottom text-sm">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="h-10 px-2 text-left align-middle font-medium text-muted-foreground">Registered</th>
