...
export const GET = apiHandler(async (request: NextRequest) => {
  ...
  const params = studentQuerySchema.parse(query);

  const service = new StudentService();
  const result = await service.findAll(schoolId, params as Record<string, unknown>);
...
export const POST = apiHandler(async (request: NextRequest) => {
  ...
  const service = new StudentService();
  const student = await service.create(validation.data, schoolId, 'system');
